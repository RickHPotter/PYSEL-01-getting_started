...
def search_google(browser, search_query):
    ...
    link = ''.join(image_url[0])
    return link

# 00. OPEN EXCEL AND WRITE IT TO A DATAFRAME
excel = 'C:/Scraping/hmm.xls'
...
for numRow in range(1, 5):
    ...
    url = search_google(browser, search_query)
    # url = ''.join(url_list)
...
# 04. WRITE TO EXCEL
df.to_excel('prototype.xlsx', engine='xlsxwriter')
